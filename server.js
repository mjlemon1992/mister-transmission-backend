...
// ── env vars (set these in Render / Railway) ─────────────────────────────────
const SM_API_KEY       = process.env.SM_API_KEY;        // Shopmonkey API key
const SM_BASE          = “https://api.shopmonkey.cloud/v3”;
const NOTIFY_EMAIL     = process.env.NOTIFY_EMAIL;      // advisor email address
...
if (!SMTP_HOST || !NOTIFY_EMAIL) return; // skip if not configured
...
try {
// 1️⃣  Create customer in Shopmonkey
...
```
// 2️⃣  Create vehicle linked to customer
...
// 3️⃣  Create a new repair order (estimate) linking customer + vehicle
...
return res.json({
  ...
  vehicleId,
  orderId,
  message: "Customer checked in and repair order created.",
});
```
